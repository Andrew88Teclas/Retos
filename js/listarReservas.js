...
function listar() {
    $.ajax({
        ...
        //url: "http://152.70.216.53:8081/api/Reservation/all",
        url: "http://152.70.216.53:8081/api/Reservation/all",
...
function listarRespuesta(items) {
    ...
    for (var i=0; i < items.length; i++) {
        ...
        */
        var texto = `Fecha inicio: ${items[i].startDate}</br>Fecha Fin: ${items[i].devolutionDate}</br>Cliente: ${items[i].client.name}</br>disfraz: ${items[i].costume.name}`;
        
        var dateOne = items[i].startDate
        var dateTwo = items[i].devolutionDate
